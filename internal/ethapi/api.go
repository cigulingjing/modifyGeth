...
func (s *BlockChainAPI) GetSecurityLevel(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (hexutil.Uint64, error) {
	...
	b := state.GetSecurityLevel(address)
...
func DoEstimateGas(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, gasCap uint64) (hexutil.Uint64, error) {
	...
	fmt.Println("DoEstimateGas", int64(*blockNrOrHash.BlockNumber))
	state, header, err := b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return 0, err
	}
...
func (s *BlockChainAPI) EstimateGas(ctx context.Context, args TransactionArgs, blockNrOrHash *rpc.BlockNumberOrHash, overrides *StateOverride) (hexutil.Uint64, error) {
	...
	// TODO：框架发来的rpc请求会指定pending
	// if blockNrOrHash != nil {
	// 	fmt.Println(blockNrOrHash)
	// 	fmt.Println("blockNrOrHash != nil")
...
func RPCMarshalHeader(head *types.Header) map[string]interface{} {
	result := map[string]interface{}{
		...
		"incentive":        (*hexutil.Big)(head.Incentive.ToBig()),
