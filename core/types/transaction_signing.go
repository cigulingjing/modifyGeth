...
type panguSigner struct{ cancunSigner }
...
func NewPanguSigner(chainId *big.Int) Signer {
	return panguSigner{cancunSigner{londonSigner{eip2930Signer{NewEIP155Signer(chainId)}}}}
}
...
func (s panguSigner) Equal(s2 Signer) bool {
	x, ok := s2.(panguSigner)
	return ok && x.chainId.Cmp(s.chainId) == 0
}
